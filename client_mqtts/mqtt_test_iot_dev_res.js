...
client.on('connect', function ()
{
    ...
    var timer = setInterval(function() {
            client.publish(topicSend, "3id=app/00112233445500000002/sub&action=command&command=get_version", {retain: false, 'QoS': 1});
            client.publish("dev/0800271C951FKzPbYtJk/sub", "2app_topic_sub=app/00112233445500000002/sub&action=command&command=get_version", {retain: false, 'QoS': 1});

            console.log('Send message server:'+count+'\n');
            count++;
            if(count == 3) {
                    clearInterval(timer);
            }
